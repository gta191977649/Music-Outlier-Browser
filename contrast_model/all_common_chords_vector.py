...
def calculate_dissonance(chord_notes):
    freqs = [pitch_to_freq(note, ref_freq=REFERENCE_FREQUENCY) for note in chord_notes]
    amps = [1] * len(chord_notes)  # Assuming equal amplitude for simplicity
    tones = [harmonic_tone(f, a) for f, a in zip(freqs, amps)]
    return dissonance(tones)
