...
import glob
import numpy as np
import h5.hdf5_getters as GETTERS
...
if __name__ == '__main__':
    h5file= "/Users/nurupo/Desktop/dev/Music-Outlier-Browser/dataset/data/blue_oyster_cult/h5/TRNJREN128F4267C9B.h5"
    # compute beat chromas
    f = GETTERS.open_h5_file_read(h5file)
    chroma = GETTERS.get_segments_pitches(f).T
    btchromas = get_btchromas(h5file)

    # compute beat timbre
    bttimbre = get_bttimbre(h5file)
    librosa.display.specshow(btchromas, y_axis='chroma', x_axis='time')
    plt.show()